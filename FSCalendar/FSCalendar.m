        ...
        if (!self.floatingMode) {
            switch (self.transitionCoordinator.representingScope) {
                case FSCalendarScopeMonth: {
                    CGFloat contentHeight = rowHeight*6 + self.padding*2;
                ...
                case FSCalendarScopeWeek: {
                    CGFloat contentHeight = rowHeight + self.padding*2;
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight, self.fs_width, contentHeight);
                    _collectionView.frame = CGRectMake(0, 0, _daysContainer.fs_width, contentHeight);
                    break;
                }
...
{
    // It is very unlikely that we'll display the 0 section and row, and for some reason, on iPad this is being
    // requested, even though we're not displaying that section, so just return a default cell to avoid crashing
    // later on for an invalid date.
    if (indexPath.section == 0 && indexPath.row == 0) {
        return [self.collectionView dequeueReusableCellWithReuseIdentifier:FSCalendarDefaultCellReuseIdentifier forIndexPath:indexPath];
    }
...
- (CGFloat)preferredRowHeight
{
    if (_preferredRowHeight == FSCalendarAutomaticDimension) {
        ...
        CGFloat padding = 5;
        if (!self.floatingMode) {
            if (self.rowHeightForPagingEnabled > CGFLOAT_EPSILON) {
                _preferredRowHeight = self.rowHeightForPagingEnabled;
            } else {
                _preferredRowHeight = (contentHeight-self.padding*2)/6.0;
            }
        } else {
